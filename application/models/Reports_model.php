...
class Reports_model extends CI_Model {
	...
	public function show_trial_balance_report(){
		extract($_POST);
		$from_date=date("Y-m-d",strtotime($from_date));
		$to_date=date("Y-m-d",strtotime($to_date));
		...
		if($view_all=="no"){
			$this->db->where("(a.updated_at>='$from_date' and a.updated_at<='$to_date')");
		}
		$this->db->from("db_account_header as a");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				echo "<tr>";
				echo "<td>".++$i."</td>";
				echo "<td>".$res1->ac_name."</td>";
				echo "<td class='text-right'>".app_number_format($res1->new_balance)."</td>";
				echo "<td class='text-right'>".app_number_format($res1->credit)."</td>";
				echo "<td class='text-right'>".app_number_format($res1->derbit)."</td>";
				echo "</tr>";
				$tot_trial_amt+=$res1->new_balance;
				$tot_trial_amt_cr+=$res1->credit;
				$tot_trial_amt_de+=$res1->derbit;
			}
			echo "<tr>
					  <td class='text-right text-bold' colspan='2'><b>Total Trial Balance :</b></td>
					  <td class='text-right text-bold'>".app_number_format($tot_trial_amt)."</td>
					  <td class='text-right text-bold'>".app_number_format($tot_trial_amt_cr)."</td>
					  <td class='text-right text-bold'>".app_number_format($tot_trial_amt_de)."</td>
				  </tr>";
		}
		else{
			echo "<tr>";
	...
	public function show_balance_sheet_report(){
		extract($_POST);
		$from_date=date("Y-m-d",strtotime($from_date));
		$to_date=date("Y-m-d",strtotime($to_date));
