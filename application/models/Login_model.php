...
class Login_model extends CI_Model
{
	...
	public function verify_credentials($username,$password)
	{
		...
		// $query=$this->db->query("select a.id,a.username,a.role_id,a.branch_id,b.role_name,from db_users a, db_roles b where b.id=a.role_id and  a.username='$username' and a.password='".md5($password)."' and a.status=1");
		$query = $this->db->query("SELECT a.id, a.username, a.role_id, a.branch_id,b.role_name,c.branch_name FROM db_users a JOIN db_roles b ON b.id = a.role_id JOIN branches c ON c.id = a.branch_id WHERE a.username = '$username' AND a.password = '" . md5($password) . "' AND a.status = 1");
		if($query->num_rows()==1){
			...
			$logdata = array('inv_username'  => $query->row()->username,
        					 ...
        					 'branch_name' =>trim($query->row()->branch_name)
				        	);
