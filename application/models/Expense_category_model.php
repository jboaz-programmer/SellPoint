...
class Expense_category_model extends CI_Model {
	...
	public function joined(){
		...
		// $query = $this->db->select('db_expense.*, db_expense_category.*')
		// ->from('db_expense')
		// ->join('db_expense_category', 'db_expense.category_id = db_expense_category.id', 'LEFT')
		// // ->join('table3 as t3', 't1.id = t3.id', 'LEFT')
		// ->get();

		// return $query;

		$query = $this->db->select('db_expense.*, db_expense_category.*,db_account_header.*,db_account_main.*');
		...
		$this->db->join('db_account_main', 'db_account_header.main_id = db_account_main.id', 'LEFT');
		$query = $this->db->get();
	...
	public function verify_and_save(){
		...
		if($query->num_rows()>0){
			...
			$query1="insert into db_expense_category(category_code,category_name,description,status,created_by,account_head) 
								values('$cat_code','$category','$description',1,'$CUR_USERNAME','$account_head')";
