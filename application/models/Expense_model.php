...
class Expense_model extends CI_Model {
	...
	public function verify_and_save(){
		...
		if ($this->db->simple_query($query1)){
			...
			$query = $this->db->select('db_expense.*, db_expense_category.*,db_account_header.new_balance,db_account_header.derbit,db_account_header.id as header_id,db_account_header.main_id');
			$this->db->from('db_expense');
			$this->db->join('db_expense_category', 'db_expense.category_id = db_expense_category.id', 'LEFT');
			$this->db->join('db_account_header', 'db_expense_category.account_head = db_account_header.id', 'LEFT');
			$this->db->where('db_expense.category_id', $category_id);
			$this->db->where('db_expense.expense_code', $expense_code);
			$last= $this->db->get();
			$q5 = $last->result();
			...
			$expense_amt=$q5[0]->expense_amt;
			$derbit=$q5[0]->derbit;
			$new_derbit = $expense_amt + $derbit;
			$header_id = $q5[0]->header_id;
			$new_balance = $q5[0]->new_balance - $expense_amt;

			$query1="update db_account_header set derbit='$new_derbit', new_balance='$new_balance' where id=$header_id";
