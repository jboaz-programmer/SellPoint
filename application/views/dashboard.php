...
<div class="wrapper">
  ...
  <div class="row" style="align-items: center;">
    <div class="col-md-6">
      <h1 style="margin: 0; padding-top: 5px;">
        <?=$page_title;?>
    ...
    <div class="col-md-6 text-right">
      <form method="post" class="form-inline" id="branch_filter_form">
        <div class="form-group">
          <label for="branch_id" class="control-label">Select Branch </label>
          <select class="form-control" id="branch_id" name="branch_id" onchange="document.getElementById('branch_filter_form').submit();">
            <?php if (!empty($branches)) { ?>
              <option value="all" <?=($branch_id == 'all') ? 'selected' : ''?>>All Branches</option>
              <?php foreach ($branches as $branch) { ?>
                <option value="<?=$branch->id;?>" <?=($branch_id == $branch->id) ? 'selected' : ''?>><?=$branch->branch_name;?></option>
              <?php } ?>
            <?php } else { ?>
