$('#save,#update').click(function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					...
					if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#account-form')[0]);//form name
						/*Check XSS Code*/
						...
						$.ajax({
						type: 'POST',
						url: base_url+'account_main/newaccount_main',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
						...
						//alert(result);
							if(result=="success")
							{
								...
								window.location=base_url+"account_main";
								return;
							}
				...
				//e.preventDefault
					...
					if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'account_main/update_account',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
{
	$.post("account/update_status",{id:id,status:status},function(result){
		if(result=="success")
				{
...
function delete_country(q_id)
{
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("account_main/delete_account",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
